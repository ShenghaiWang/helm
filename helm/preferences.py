...
KEY_MODEL_EXCLUDE = "model.exclude"
...
SUPPORTED_KEYS: dict[str, tuple[bool, str]] = {
    ...
    KEY_MODEL_EXCLUDE: (
        True,
        "exact model ids this root will never use, whatever a runtime's own "
        "catalogue advertises (a model the account cannot actually run)",
    ),
    KEY_EFFORT_DEFAULT: (
        False,
...
class Preferences:
    ...
    #: Exact model ids this root refuses, whatever a runtime advertises.
    #:
    #: Distinct from `model.runtimes`, which says WHERE a family may run. This
    #: says a specific id is unusable here at all. It exists because a
    #: runtime's own catalogue can list a model the account cannot actually
    #: reach: `opencode models` advertises `openai/gpt-5.4`, and selecting it
    #: fails with "not supported when using Codex with a ChatGPT account".
    #: Helm was doing the right thing -- enumerating what the machine reports
    #: rather than inventing an id -- and the enumeration was wrong.
    #:
    #: The failure is why this is worth a preference rather than a note. The
    #: launch succeeds, the agent prints the refusal and then idles: alive to
    #: every liveness check, reporting nothing, until the stall detector gives
    #: up ~1,600s later. Nine reviewer tasks on this root drew that model.
    ...
    def entries(self) -> list[tuple[str, str]]:
        ...
        if self.excluded_models:
            rows.append((KEY_MODEL_EXCLUDE, ", ".join(sorted(self.excluded_models))))
        if self.default_model:
...
def _from_document(document: Any, path: Path | None) -> Preferences:
    ...
    excluded_models = frozenset(
        ...
        # that accepted anything would let a typo sit in the file looking like
        # a restriction while excluding nothing.
        _model_id(item, "model.exclude", where)
        for item in _list(model.get("exclude"), "model.exclude", where)
    )
...
def apply(current: Preferences, key: str, values: Iterable[str] | None) -> Preferences:
    ...
    elif key == KEY_MODEL_EXCLUDE:
        if listed is None:
            model.pop("exclude", None)
        else:
            model["exclude"] = sorted(set(listed))
