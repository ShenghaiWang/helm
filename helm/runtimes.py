...
# The project repository's shared Git directory. A task worktree's `.git` is a
# file pointing into it, so every commit writes there, outside the worktree.
...
BUILTIN_RUNTIMES: tuple[AgentRuntime, ...] = (
    AgentRuntime(
        ...
        # --settings carries the one hook Helm needs in a worker: a
        # SessionStart line telling the session to arm a watch on its inbox,
        # the same way this repository's own hook arms the root's watch. It
        # merges with the project's own .claude settings rather than
        # replacing them, and it is written per worker so nothing lands in
    ...
    AgentRuntime(
        ...
        # workspace plus exactly the named directories, which is the boundary
        # that was always intended.
        # The second --add-dir is the project's shared Git directory. A task
        # worktree's `.git` is a file pointing at `<repo>/.git/worktrees/<id>`,
        # and a commit writes the index there and objects and refs into the
        # common dir -- both outside the worktree the sandbox allows. Without
        # it a codex author edits freely, then cannot commit: "sandbox denies
        # shared Git index write", and the round ends as a blocker with the
...
def apply_prompt(
    ...
    reads only ``HELM_CONTEXT_FILE`` keeps working exactly as before. A path
    placeholder with no value is dropped together with the flag in front of
    it, because an empty writable root or settings file is a launch error,
    not a narrower one.
    """
    filled = {
        PROMPT_PLACEHOLDER: prompt,
    ...
    for part in command:
        if part in filled and part != PROMPT_PLACEHOLDER and not filled[part]:
            if out and out[-1] in _PATH_FLAGS:
                out.pop()
            continue
        out.append(filled.get(part, part))
    return out
