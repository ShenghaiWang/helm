...
class SubprocessHerdrClient:
    ...
    #: A worker's pane starts an interactive shell, and the launch command is
    #: typed into it. Anything the shell's startup prints and waits on eats
    #: the first keystrokes: oh-my-zsh's "Would you like to update? [Y/n]"
    #: turned `env PYTHONPATH=...` into `nv PYTHONPATH=...`, the runner never
    #: ran, and Helm reported "never started in its pane" -- twice in one
    #: morning. Shells that read this variable skip the check; shells that do
    #: not see an unused variable.
    TAB_ENVIRONMENT = (("DISABLE_AUTO_UPDATE", "true"),)
...
class HerdrAdapter:
    ...
    def launch_task(
        ...
        *,
        ...
        # The pane is up and the runner is in it, so the agent's own process
        # exists and can be found. Take its pid now: it is the only witness
        # that survives the pane being closed, and without it a closed pane is
        # indistinguishable from a finished agent.
        with contextlib.suppress(HelmError, SafetyError, OSError):
            self.coordinator.adopt_worker_pid(worker["id"])
    ...
    def _provider_worker_alive(self, worker: dict[str, Any]) -> bool | None:
        ...
        A pane that still exists is not a live session: the agent inside it
        can be gone with the shell prompt showing. So a recorded pid that is
        dead answers before the pane is even asked -- it is the agent's own
        process, adopted at launch, and its absence is the fact.
        """
        pid = worker.get("pid")
        if isinstance(pid, int) and pid > 0 and not self.coordinator._pid_alive(pid):
            return False
        if not self.client.available():
            return False
        data = self.coordinator.store.load()
    ...
    def _verdict_from_output(
        ...
        for index in range(len(lines) - 1, -1, -1):
            ...
            if instruction and candidate and candidate in instruction:
                continue
            if instruction and self._is_garbled_fragment_of(candidate, instruction):
                # The exact-substring guard above was defeated by the terminal
                # itself: a TUI redrawing under load drops characters, so the
                # displayed brief read "wheterthe loop oninues" and no longer
                # matched the instruction it was. A pane is a lossy surface;
                # recognising the instruction has to tolerate the loss.
                continue
    ...
    #: Share of a candidate line's letter 5-grams that must reappear in the
    #: instruction for the line to count as the instruction, garbled. A real
    #: verdict names files and findings the brief never mentions and lands
    #: well below this; a dropped character every dozen or so leaves most
    #: 5-grams intact and lands well above it.
    GARBLE_MATCH_SHARE = 0.6

    @classmethod
    def _is_garbled_fragment_of(cls, candidate: str, instruction: str) -> bool:
        letters = lambda text: re.sub(r"[^a-z0-9]", "", text.lower())  # noqa: E731
        sample = letters(candidate)[:160]
        haystack = letters(instruction)
        if len(sample) < 20 or not haystack:
            return False
        grams = [sample[i:i + 5] for i in range(len(sample) - 4)]
        hits = sum(1 for gram in grams if gram in haystack)
        return hits / len(grams) >= cls.GARBLE_MATCH_SHARE
    ...
    def release_finished_tabs(self) -> list[str]:
        ...
        for worker_id, layout in list(state.get("workers", {}).items()):
            ...
            # Last look at the pane before it goes: its session id names the
            # transcript the task's cost is read from.
            if layout.get("pane_id"):
                self._agent_status(layout["pane_id"], worker_id)
    ...
    def _full_suite_evidence(cls, data: dict[str, Any], task_id: str) -> str:
        ...
        *Every* report, not merely the last one. A suite is not always one
        ...
        if not filed:
            return (
                "\n\nAUTHOR'S TEST EVIDENCE: NO MACHINE-READABLE PAYLOAD -- no "
                "message on this task carries the `full_suite` payload key. That is NOT "
                "the same as no tests having run, and the difference has already cost a "
                ...
                "blocking finding. Targeted tests are the standard -- a missing "
                "FULL-suite run is never the finding, since CI runs it on the pull "
    ...
    #: What Herdr's runtime hooks say the agent in a pane is doing. Only these
    #: carry meaning here; anything else is `unknown` and falls back to
    #: watching the output for a gap.
    AGENT_STATUSES = ("idle", "working", "blocked")
    #: What the last `answer_worker` call did, for callers that report rather
    #: than gate: typed, nudged, watched, blocked, unconfirmed, unreachable.
    last_wake_outcome = "unreachable"

    def _agent_status(self, pane_id: str, worker_id: str | None = None) -> str:
        """What the agent in this pane is doing, from the runtime's own hook.

        Herdr's integrations (claude, pi, omp) report `idle`, `working` or
        `blocked` -- a permission or trust dialog -- through `pane
        report-agent`, so for those runtimes this is a fact where
        `_wait_for_quiet` is a guess from output growth: a foreman blocked
        inside `helm review` is silent and busy at once. A runtime with no
        integration reports nothing, which comes back as `unknown`.
        """
        status_call = getattr(self.client, "pane_status", None)
        if status_call is None:
            return "unknown"
        try:
            reply = status_call(pane_id)
        except (HelmError, OSError, subprocess.SubprocessError):
            return "unknown"
        pane = ((reply or {}).get("result") or {}).get("pane") or {}
        if worker_id:
            # The same reply names the runtime's session, which is what the
            # transcript -- and so the task's cost -- is filed under.
            session = pane.get("agent_session") or {}
            if isinstance(session, dict) and session.get("kind") == "id":
                with contextlib.suppress(HelmError, OSError):
                    self.coordinator.note_agent_session(
                        worker_id, str(session.get("value") or "")
                    )
        status = str(pane.get("agent_status") or "").strip().lower()
        return status if status in self.AGENT_STATUSES else "unknown"

    def _pointer_line(self, worker_id: str, text: str, note: Path | None) -> str:
        """One line for the pane: what arrived, and where the whole of it is."""
        gist = self._message_gist(text)
        head = (
            f"Helm instructs you: {gist}" if gist else "Helm has an instruction for you."
        )
        where = f" Full message in {note}:" if note is not None else ""
        return (
            f"{head}{where} read it now and act on it as if it had been typed "
            "here; it stays unread until you run your inbox command "
            "(usage_inbox in your assignment)."
        )

    def answer_worker(
        self, worker_id: str, text: str, *, note: Path | None = None
    ) -> str:
        """Deliver a coordinator message: the inbox is the message, the pane a wake.

        Returns how the worker was woken -- `typed` (the whole text went into
        an idle session and was taken, so the note is marked read), `nudged`
        (one pointer line went in and was taken; the note stays unread until
        the worker reads it), `watched` (a live harness watch on the inbox
        will wake it, so nothing is typed) -- or "" when Helm cannot say the
        agent will see the note before its next helm command: no session to
        speak to, a dialog up (`Enter` would answer the dialog), or a line
        typed that nothing confirmed. `last_wake_outcome` names which, for a
        caller that reports rather than gates.

        Nothing here presses Escape any more. Delivery used to open with one,
        to clear an interrupt prompt a paste was believed to cause -- and the
        Escape was the interrupt: it cancelled whatever tool call the agent was
        inside, which is how a foreman lost the `helm review` it was blocked
        on, three times in one afternoon. `pane send-text` fills an input
        buffer and a separate `Enter` submits it; sent together the newline
        races the paste, hence the pause between them.
        """
        self.last_wake_outcome = "unreachable"
        if note is None:
            with contextlib.suppress(HelmError, OSError):
        ...
        self._owned(layout, f"tab for worker {worker_id}")
        pane = layout["pane_id"]
        if self.coordinator.inbox_watch_live(worker_id):
            # Its harness is watching the inbox and will wake it with the note;
            # a typed line would be a second copy landing at a worse moment.
            self.last_wake_outcome = "watched"
            return "watched"
        status = self._agent_status(pane, worker_id)
        if status == "blocked":
            # A dialog is up. Enter would answer the dialog, not deliver this.
            self.last_wake_outcome = "blocked"
            return ""
        # Wait for a gap so the line lands between the agent's own output rather
        # than through the middle of it. Bounded: a busy agent still gets the
        # pointer, because the alternative was a message nobody was told about.
        quiet = True if status == "idle" else self._wait_for_quiet(worker_id)
        idle = status == "idle" or (status == "unknown" and quiet)
        # A short message to an idle agent is typed whole: it acts on it at
        # once, with no command to run first. Everything else -- long, or an
        # agent mid-turn -- gets the one-line pointer, because a body pasted
        # into a redrawing TUI interleaves with it character by character and
        # the pane stops being evidence of anything.
        inline = idle and len(text) <= self.INLINE_MESSAGE_LIMIT
        send_text(pane, text if inline else self._pointer_line(worker_id, text, note))
        # A pause before Enter: sent immediately, the newline races the text.
        ...
        for _attempt in range(self.SUBMIT_ATTEMPTS):
            ...
            if self._submitted(worker_id, before):
                if inline and note is not None:
                    # The whole text went in and the session acted on it; that
                    # is the note read, by the only reader there is.
                    self.coordinator.mark_inbox_read(worker_id, note)
                self.last_wake_outcome = "typed" if inline else "nudged"
                return self.last_wake_outcome
        # The line is in a buffer nothing took. Silence must not read as
        # success; the note is still there and the worker's next command
        # prints it, which is what `pending` watches for.
    ...
    def interrupt_worker(self, worker_id: str) -> bool:
        ...
        This is the one thing `answer_worker` no longer does: stop what the
        agent is doing without stopping the agent. It is its own verb because
        a message and an interruption are different acts, and conflating them
        was the whole bug.
        """
        if not self.client.available():
            return False
        ...
        self._owned(layout, f"tab for worker {worker_id}")
        with contextlib.suppress(HerdrUnavailable):
            send_keys(layout["pane_id"], "Escape")
            return True
        return False
    ...
    def _process_still_running(self, worker: dict[str, Any]) -> bool:
        """Whether the agent behind a vanished pane is still executing.

        The provider answers about a pane. This answers about a process, and
        the two disagree in the case that matters: a pane the user closed, or
        one a provider restart forgot, over an agent that is still working.
        Recording that as `lost` is wrong twice -- it fails a task that has not
        failed, and it makes the still-running agent unaddressable, because
        every command that would reach it first checks the record.

        A record with no pid (an older worker, or a scan that found nothing)
        answers False, which keeps the previous behaviour exactly. This can
        only ever spare a worker that is demonstrably alive.
        """
        pid = worker.get("pid")
        if not pid:
            pid = self.coordinator.adopt_worker_pid(worker["id"])
        return bool(pid) and self.coordinator._pid_alive(pid)
    ...
    def poll_worker(self, worker_id: str) -> dict[str, Any]:
        ...
        if worker.get("execution") == "herdr" and worker.get("status") == "running":
            alive = self._provider_worker_alive(worker)
            if alive is False and not self._process_still_running(worker):
