...
def _build_parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--model",
        help="model this task runs on; overrides the project pin and HELM_MODEL. "
             ...
             "which is otherwise unreachable once a root default is set")
    ...
    cost = task_commands.add_parser(
        "cost",
        help=(
            "what a task's sessions consumed, from the runtimes' own transcripts; "
            "tokens, and dollars only where a run reported them"
        ),
    )
    cost.add_argument("task_id")
    cost.add_argument("--no-reviews", action="store_true", help="leave the reviewer tasks out")
    cost.add_argument("--json", action="store_true", dest="as_json")
    ...
    report.add_argument(
        ...
        help=(
            "with --type question: block until Helm answers and print the answer, "
            "up to SECONDS (default 100, under a harness tool timeout); exit 3 when "
            "it is still unanswered, and wait again with helm worker inbox --wait"
        ),
    )
    ...
    inbox = worker_commands.add_parser(
        "inbox",
        help=(
            "read the messages Helm left for a worker and mark them read; "
            "a worker reads its own"
        ),
    )
    inbox.add_argument("worker_id", nargs="?", help="defaults to the calling worker")
    inbox.add_argument(
        "--changes", action="store_true",
        help="print only what arrived since the last read, and nothing when nothing did -- for a watch loop",
    )
    inbox.add_argument(
        "--wait", type=float, metavar="SECONDS",
        help="block until a message arrives or SECONDS pass (exit 3 on timeout)",
    )
    inbox.add_argument(
        "--peek", action="store_true", help="show unread messages without marking them read",
    )

    interrupt = worker_commands.add_parser(
        "interrupt",
        help=(
            "send Escape into a worker's session, deliberately: stop what it is "
            "doing without stopping it"
        ),
    )
    interrupt.add_argument("worker_id")
    ...
    corpus_add = eval_commands.add_parser("add", help="register a closed ticket in the corpus")
    corpus_add.add_argument("ticket")
    corpus_add.add_argument("--base", required=True, help="commit the shipped fix started from")
    corpus_add.add_argument("--merge", required=True, help="commit the shipped fix landed as")
    corpus_add.add_argument("--brief-file", required=True, help="file holding the ticket's own description")
    corpus_add.add_argument("--title", default="")
    corpus_add.add_argument("--pr", type=int)
    eval_commands.add_parser("list", help="the corpus and every run's state")
    settings_cmd = eval_commands.add_parser(
        "settings", help="merge a JSON file into the corpus settings (projects, repo, judge, checks)"
    )
    settings_cmd.add_argument("file")
    ...
    run_cmd.add_argument(
        ...
        help="abandon a run still marked running (its record is kept aside) and start again",
    )
    status_cmd = eval_commands.add_parser("status", help="collect finished runs and judges, then show them")
    status_cmd.add_argument("ticket", nargs="?")
    checks_cmd = eval_commands.add_parser("checks", help="run the corpus check commands in a run's checkout")
    checks_cmd.add_argument("ticket")
    checks_cmd.add_argument("--arm", required=True, choices=evaluation_module.ARMS)
    judge_cmd = eval_commands.add_parser("judge", help="score a run's candidate against the shipped change")
    judge_cmd.add_argument("ticket")
    judge_cmd.add_argument("--arm", required=True, choices=evaluation_module.ARMS)
    judge_cmd.add_argument(
        "--anyway", action="store_true",
        help="judge a run marked contaminated; the contamination stays on its record",
    )
    sanitize_cmd = eval_commands.add_parser(
        "sanitize",
        help=(
            "strip an arm's repository to the history a ticket starts from -- what every "
            "run does first; here to do it by hand and see what it removes"
        ),
    )
    sanitize_cmd.add_argument("ticket")
    sanitize_cmd.add_argument("--arm", required=True, choices=evaluation_module.ARMS)
    note_cmd = eval_commands.add_parser(
        "note", help="attach a note to a run -- a confound, an incident, anything the numbers need beside them"
    )
    note_cmd.add_argument("ticket")
    note_cmd.add_argument("--arm", required=True, choices=evaluation_module.ARMS)
    ...
    watchdog_commands.add_parser(
        "restart",
        help="restart the running watchdog so it picks up the code on disk",
    )
    ...
    ask = commands.add_parser(
        "ask",
        help="record that the coordinator put a question to the commander, "
             "and report what has been asked",
    )
    ask_commands = ask.add_subparsers(dest="ask_command", required=True)
    ask_record = ask_commands.add_parser(
        "record", help="record one question put to the commander"
    )
    ask_record.add_argument(
        "project_id",
        nargs="?",
        default=None,
        help="optional; omit for an ask that belongs to no single project",
    )
    ask_record.add_argument(
        "--reason",
        required=True,
        choices=list(COMMANDER_ASK_REASONS),
        help="authorization: a protected action only a human may decide. "
             "ambiguity: the brief did not say and the coordinator could not "
             "resolve it. escalation: a worker's obstacle arriving upward.",
    )
    ask_record.add_argument("--text", required=True, help="what was asked")
    ask_record.add_argument("--task", dest="task_id", default=None)
    ask_show = ask_commands.add_parser(
        "show", help="what has been asked of the commander, by reason"
    )
    ask_show.add_argument("project_id", nargs="?", default=None)
    ...
    review.add_argument("--reviewer-model",
        help="review on a different model of the same runtime. Pass 'runtime' to "
             "leave the choice to the reviewer runtime's own default -- a root "
...
def _detach_runner() -> None:
    """Leave the launcher's process tree before the worker starts.

    The launcher already puts the runner in its own session, which keeps a
    signal to the caller's process group away from the worker. It does not
    keep away a caller that walks the tree by parent pid: an agent harness
    interrupting the tool call that ran `helm review` killed the reviewer
    child two seconds after launch, twice, while the runner lived to write
    the exit record. So the runner forks once more and the child carries on
    reparented to init, where no walk from the caller can reach it. The
    parent's only job is to print the child's pid so the launcher records
    the process that is actually running, then exit.
    """
    pid = os.fork()
    if pid:
...
def _eval_command(coordinator: Coordinator, args: argparse.Namespace) -> int:
    evaluation = evaluation_module.Evaluation(coordinator.store.directory / "evaluation")
    runner = evaluation_module.Runner(coordinator, evaluation)
    if args.eval_command == "add":
        brief = Path(args.brief_file).read_text(encoding="utf-8")
        entry = evaluation.add_ticket(
            args.ticket, base=args.base, merge=args.merge, brief=brief,
            title=args.title, pr=args.pr,
        )
        print(f"Registered {entry['id']} base={entry['base'][:10]} merge={entry['merge'][:10]}")
        return 0
    if args.eval_command == "settings":
        incoming = json.loads(Path(args.file).read_text(encoding="utf-8"))
        if not isinstance(incoming, dict):
            raise HelmError("settings must be a JSON object")
        corpus = evaluation.load_corpus()
        corpus["settings"].update(incoming)
        evaluation.save_corpus(corpus)
        print(f"Settings now: {json.dumps(corpus['settings'], indent=2)}")
        return 0
    if args.eval_command == "list":
        corpus = evaluation.load_corpus()
        if not corpus["tickets"]:
            print("The corpus is empty; register tickets with helm eval add.")
            return 0
        for entry in corpus["tickets"]:
            states = []
            for arm in evaluation_module.ARMS:
                run = evaluation.load_run(entry["id"], arm)
                states.append(f"{arm}={run['status'] if run else '-'}")
            print(
                f"{entry['id']:<10} as {entry['alias']:<8} {entry['base'][:10]}..{entry['merge'][:10]}  "
                f"{'  '.join(states)}  {entry.get('title', '')}"
            )
        return 0
    if args.eval_command == "run":
        if args.arm == "single":
            record = runner.start_single(
            ...
            def route(project_id: str, text: str) -> None:
                ...
                if code != 0:
                    raise HelmError(f"routing to {project_id} failed (exit {code})")
            record = runner.start_helm(args.ticket, route=route, restart=args.restart)
            print(f"Started {args.ticket} on helm: routed to project {record['project_id']}")
        print("  helm eval status collects it when its sessions have said their last word.")
        return 0
    if args.eval_command == "status":
        corpus = evaluation.load_corpus()
        tickets = [t for t in corpus["tickets"] if not args.ticket or t["id"] == args.ticket]
        adapter = HerdrAdapter(coordinator)
        for entry in tickets:
            for arm in evaluation_module.ARMS:
                record = runner.collect(entry["id"], arm, adapter=adapter)
                if record is None:
                    continue
                record = runner.collect_judge(entry["id"], arm) or record
                judge = record.get("judge") or {}
                metrics = record.get("metrics") or {}
                print(
                    f"{entry['id']} {arm:<9} {record.get('status'):<12} "
                    f"tip={str(record.get('tip') or '-')[:10]:<10} "
                    f"judge={judge.get('score') if judge else '-'} "
                    f"wall={int(metrics.get('wall_seconds') or 0) // 60}m "
                    f"interventions={metrics.get('interventions') or {}}"
                )
                if record.get("contamination"):
                    print(f"  contaminated: {'; '.join(record['contamination'])}")
        return 0
    if args.eval_command == "checks":
        results = runner.run_checks(args.ticket, args.arm)
        for result in results:
            print(f"  {'ok  ' if result['ok'] else 'FAIL'} {result['command']} ({result['seconds']}s)")
        return 0 if all(r["ok"] for r in results) else 1
    if args.eval_command == "judge":
        record = runner.start_judge(
            args.ticket, args.arm, adapter=HerdrAdapter(coordinator), anyway=args.anyway
        )
        print(f"Judge launched for {args.ticket} on {args.arm}: task={record['judge_task_id']}")
        return 0
    if args.eval_command == "sanitize":
        removed = runner.sanitize(args.ticket, args.arm)
        for kind, items in removed.items():
            if items:
                print(f"{kind} removed ({len(items)}): {', '.join(items[:12])}{' …' if len(items) > 12 else ''}")
        if not any(removed.values()):
            print("Nothing to remove.")
        return 0
    if args.eval_command == "note":
        record = evaluation.load_run(args.ticket, args.arm)
        if record is None:
            raise HelmError(f"no run recorded for {args.ticket} on {args.arm}")
        record.setdefault("notes", []).append({"at": evaluation_module._now(), "text": args.text})
        evaluation.save_run(record)
        print(f"Noted on {args.ticket}/{args.arm}: {args.text}")
        return 0
    if args.eval_command == "report":
        text = evaluation.report()
        report_file = evaluation.directory / "report.md"
        _write_private_text(report_file, text)
        print(text, end="")
        print(f"(written to {report_file})")
        return 0
    raise HelmError(f"unknown eval command {args.eval_command}")


#: How long a message may sit unread in a running worker's inbox before
#: `pending` names it. Long enough for one real command to finish; short
...
_INBOX_DELIVERY_WORDS = {
    "typed": "delivered: typed whole into its idle session",
    "nudged": "in its inbox; one pointer line typed into its session and taken",
    "watched": "in its inbox; its own watch will deliver it",
    "blocked": "in its inbox; its session is at a dialog, so nothing was typed -- "
...
def _format_inbox(worker_id: str, notes: list[dict]) -> str:
    """The notes as a worker reads them: stamped, delimited, and in order."""
    count = len(notes)
    lines = [
        f"[helm inbox] {count} message{'s' if count != 1 else ''} for {worker_id} "
        "-- read and act on them before continuing:"
    ]
    for note in notes:
        age = int(note.get("age_seconds") or 0)
        waited = f"{age // 60}m" if age >= 60 else f"{age}s"
        lines.append(f"--- {note['id']} ({note['created_at']}, {waited} ago) ---")
        lines.append(str(note.get("text", "")).rstrip("\n"))
    lines.append("--- end of inbox ---")
    return "\n".join(lines) + "\n"


def _print_agent_inbox(coordinator: Coordinator, args: argparse.Namespace) -> None:
    """Put a worker's unread inbox at the top of whatever it asked for.

    A worker calls `helm` constantly -- to push status, to ask, to review --
    so its next command is the earliest moment it can act on a message, on
    every runtime, with no hook and no discipline to remember. The note is
    marked read here because the output goes to the same session that runs
    the command: the reader is the caller.
    """
    if args.command == "worker" and getattr(args, "worker_command", None) == "inbox":
        return
    marked = os.environ.get("HELM_WORKER_ID", "").strip()
    if not marked:
        return
    with contextlib.suppress(HelmError, OSError):
        notes = coordinator.read_inbox(marked)
        if notes:
...
_ROOT_ONLY_COMMANDS = frozenset({
    ...
    # experiment, not something a worker runs on itself.
    ("eval", None),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        _print_agent_inbox(coordinator, args)
