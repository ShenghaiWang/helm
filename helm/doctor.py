...
def _elapsed_to_start(etime: str) -> float | None:
    """Turn ps's `[[DD-]HH:]MM:SS` elapsed time into a start timestamp."""
...
class _Doctor:
    ...
    def _check_watchdog_freshness(self, entry: "object") -> None:
        """Is the RUNNING watchdog the code that is on disk?

        The watchdog is a long-lived daemon: it imports Helm once at startup
        and keeps that copy for its whole life. So a fix to `helm/watchdog.py`
        has no effect on what the human actually receives until the daemon is
        restarted, and nothing about the outside says so -- git says fixed, the
        tests say fixed, and the process quietly disagrees.

        That is not hypothetical. The notification body was corrected here and
        the commander kept receiving the old, contentless one, because the
        agent had been running for a day and a half. The only way to notice was
        to compare a screenshot against the code.

        A daemon older than the module it is running is therefore a warning:
        the mechanism reports healthy while delivering the previous build.
        """
        import subprocess as _subprocess
        from pathlib import Path as _Path

        # `package_parent()`, not `Path(__file__).parent`. Deriving a path from
        # where this file happens to sit means a different directory the moment
        # the file moves, and that trap has already cost this repository a
        # broken release once -- which is why there is an architecture guard
        # for it, and why that guard caught this line before it shipped.
        from .paths import package_parent

        module = package_parent() / "helm" / "watchdog.py"
        if not module.exists():
            self.ok("root.watchdog", f"watchdog scheduler entry present ({entry.name})")
            return
        started: float | None = None
        try:
            # `ps -o lstart=` is the portable-enough answer; etime gives an
            # elapsed string that would have to be parsed by hand.
            found = _subprocess.run(
                ["pgrep", "-f", "helm.*watchdog run"],
                capture_output=True, text=True, timeout=10, check=False,
            )
            pid = (found.stdout or "").split()
            if pid:
                # `etime`, and not the two more obvious choices. BSD ps --
                # what macOS ships -- has no `etimes` keyword at all and fails
                # with "keyword not found", which made this read as "no
                # watchdog is running" on every Mac: the same silent-wrong
                # answer the check exists to catch. `lstart` does exist but
                # prints a locale-formatted date ("Thu 10 Sep ..." here, day
                # before month), so parsing it breaks on someone else's
                # machine. `etime` is [[DD-]HH:]MM:SS everywhere.
                begun = _subprocess.run(
                    ["ps", "-o", "etime=", "-p", pid[0]],
                    capture_output=True, text=True, timeout=10, check=False,
                )
                started = _elapsed_to_start(begun.stdout or "")
        except Exception:  # noqa: BLE001 - a doctor check never fails the root
            started = None
        if started is None:
            self.warn(
                "root.watchdog",
                f"watchdog entry present ({entry.name}) but no running process was "
                "found: nothing reaches a human while no session is open",
                "run helm watchdog restart",
            )
            return
        if module.stat().st_mtime > started:
            self.warn(
                "root.watchdog",
                "the running watchdog started before the current helm/watchdog.py "
                "and is still executing the previous build, so notification "
                "changes have not taken effect",
                "run helm watchdog restart",
            )
            return
        self.ok("root.watchdog", f"watchdog running and current ({entry.name})")
