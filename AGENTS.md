...
**refuses a stated level** rather than dropping it silently — the task's, the
project's, or `HELM_EFFORT`. A root `effort.default` is a floor for runtimes
that take one, not a demand: on such a runtime it is dropped and the task
records that it launched at the runtime's own default. A root can
