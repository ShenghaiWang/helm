...
class TaskLifecycleTests(HelmTestCase):
    ...
    def test_concurrent_base_branch_change_during_resolution_forces_a_retry(self) -> None:
        ...
        with mock.patch("helm.git._resolve_task_base", side_effect=racing):
            task = self.coordinator.create_task(
...
class ATaskThatNeverRanCanStillBeShedTests(HelmTestCase):
    """A created-but-never-launched task holds a checkout and nothing else.

    Appointing a foreman creates one every time, and replacing a stuck foreman
    leaves the old one behind. They accumulated with no way to shed them:
    cleanup demanded a *terminal* worker, and a task with no worker can never
    have one, so the refusal could never be satisfied by waiting.
    """

    def _never_started(self, name: str) -> tuple[dict, dict]:
        root = self.repo(name)
        project = self.coordinator.register_project(
            name.title(), str(root), project_id=name
        )
        ...
        self.coordinator.allocate_task(task["id"])
        return project, self.coordinator.store.load()["tasks"][task["id"]]

    def test_cleanup_sheds_a_task_that_never_launched_a_worker(self) -> None:
        _, task = self._never_started("neverran")
        workspace = Path(task["workspace"])
        self.assertTrue(workspace.is_dir(), "precondition: it holds a checkout")

        self.coordinator.cleanup_task(task["id"])

        self.assertFalse(workspace.exists())
        self.assertTrue(
            self.coordinator.store.load()["tasks"][task["id"]]["workspace_removed"]
        )

    def test_approve_still_refuses_a_task_that_never_ran(self) -> None:
        """The relaxation is cleanup's alone.

        Approve, merge and learning proposals ask whether work STANDS. A task
        that never ran has no work to stand, so accepting one there would
        approve an empty tree.
        """
        _, task = self._never_started("neverapprove")
        with self.assertRaises((SafetyError, HelmError)):
            self.coordinator.approve_task(task["id"])

    def test_merge_still_refuses_a_task_that_never_ran(self) -> None:
        _, task = self._never_started("nevermerge")
        with self.assertRaises((SafetyError, HelmError)):
            self.coordinator.merge_task(task["id"])

    def test_cleanup_still_refuses_while_a_worker_is_running(self) -> None:
        """Relaxing "no worker" must not relax "a worker is still working"."""
        root = self.repo("liveworker")
        project = self.coordinator.register_project(
            "Liveworker", str(root), project_id="liveworker"
        )
        task = self.coordinator.create_task(project["id"], "long work")
        worker = self.coordinator.launch_worker(
            task["id"], [sys.executable, "-c", "import time; time.sleep(120)"], wait=False
        )
        self.addCleanup(self._kill_pid, worker.get("pid"))
...
class OneTicketOneWorktreeTests(HelmTestCase):
    def test_a_second_task_for_a_ticket_is_refused_toward_continue(self) -> None:
        root = self.repo("tickets")
        project = self.coordinator.register_project("Tickets", str(root), project_id="tickets")
        first = self.coordinator.create_task(project["id"], "do T-7", ticket="T-7")
        self.coordinator.allocate_task(first["id"])
        with self.assertRaisesRegex(HelmError, rf"helm task continue {first['id']}"):
            self.coordinator.create_task(project["id"], "do T-7 again", ticket="T-7")
        # A read-only look at the same ticket is not a second line of work.
        self.coordinator.create_task(project["id"], "inspect T-7", ticket="T-7", read_only=True)
        # And a deliberate second line says so.
        second = self.coordinator.create_task(project["id"], "do T-7 elsewhere", ticket="T-7", new=True)
        self.assertNotEqual(second["id"], first["id"])

    def test_a_failed_task_points_at_reopen_and_a_cleaned_one_no_longer_blocks(self) -> None:
        root = self.repo("failed-ticket")
        project = self.coordinator.register_project("Failed", str(root), project_id="failed-ticket")
        task = self.coordinator.create_task(project["id"], "do T-8", ticket="T-8")
        self.coordinator.allocate_task(task["id"])
        with self.state.locked() as data:
            data["tasks"][task["id"]]["status"] = "failed"
        with self.assertRaisesRegex(HelmError, rf"helm task reopen {task['id']}, then helm task continue"):
            self.coordinator.create_task(project["id"], "retry T-8", ticket="T-8")
        # Once the worktree is gone, the ticket is free again.
        shutil.rmtree(task["workspace"], ignore_errors=True)
        self.coordinator.create_task(project["id"], "retry T-8", ticket="T-8")


class AnOpenPullRequestCanTakeAnotherRoundTests(HelmTestCase):
    def test_a_pr_open_task_continues_into_a_new_round(self) -> None:
        root = self.repo("pr-round")
        project = self.coordinator.register_project("PrRound", str(root), project_id="pr-round")
        task = self.coordinator.create_task(project["id"], "ship it", ticket="T-3")
        self.coordinator.allocate_task(task["id"])
        with self.state.locked() as data:
            data["tasks"][task["id"]]["status"] = "pr-open"
