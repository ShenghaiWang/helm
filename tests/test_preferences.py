...
class ExcludedModelsTests(HelmTestCase):
    """A runtime's own catalogue can advertise a model the account cannot run.

    `opencode models` lists `openai/gpt-5.4`; selecting it fails with "not
    supported when using Codex with a ChatGPT account". Helm was doing the
    right thing -- enumerating what the machine reports rather than inventing
    an id -- and the enumeration was wrong. Nine reviewer tasks on one root
    drew that model before there was any way to record that it is unusable.

    What made it worth a preference rather than a note is the failure shape:
    the launch succeeds, the agent prints the refusal and idles, alive to every
    liveness check and reporting nothing, until the stall detector gives up
    ~1,600s later. A whole review round is lost to a pairing that could never
    have worked.
    """

    def test_an_excluded_model_is_refused_however_it_is_named(self) -> None:
        """Including by a task's own choice.

        `model.exclude` records a fact about the machine, not a preference
        about defaults. An exclusion that an explicit choice could step over
        would not have stopped any of the tasks it exists for -- every one of
        them named the model explicitly.
        """
        self.write_preferences(**{"model": {"exclude": ["openai/gpt-5.4"]}})
        project = {"id": "excl"}
        for source in ({"model": "openai/gpt-5.4"},):
            with self.assertRaises(HelmError) as caught:
                self.coordinator._resolve_model(project, source)
            self.assertIn("model.exclude", str(caught.exception))

    def test_a_model_that_is_not_excluded_still_resolves(self) -> None:
        self.write_preferences(**{"model": {"exclude": ["openai/gpt-5.4"]}})
        model, reason = self.coordinator._resolve_model(
            {"id": "excl"}, {"model": "openai/gpt-5.6-sol"}
        )
        self.assertEqual(model, "openai/gpt-5.6-sol")
        self.assertIn("task names model", reason)

    def test_the_exclusion_survives_a_write_and_read(self) -> None:
        """It is a standing fact, so it has to outlive the process that set it."""
        self.write_preferences(**{"model": {"exclude": ["openai/gpt-5.4"]}})
        loaded = self.coordinator.preferences()
        self.assertIn("openai/gpt-5.4", loaded.excluded_models)

    def test_an_unusable_id_in_the_exclusion_is_refused_at_load(self) -> None:
        """A typo would sit in the file looking like a restriction and exclude nothing."""
        from helm.preferences import PreferencesError

        with self.assertRaises((PreferencesError, HelmError)):
            self.write_preferences(**{"model": {"exclude": [""]}})
            self.coordinator.preferences()
