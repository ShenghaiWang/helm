...
class RouteCommandTests(HelmTestCase):
    ...
    def test_pending_surfaces_a_request_the_foreman_has_not_acted_on(self) -> None:
        ...
        def pending_text() -> str:
            # The subprocess runs with `cwd` inside a temporary Helm root, so
            # the repository is not on its import path and `-m helm` used to
            # die with "No module named helm". Every assertion below then
            # matched against a traceback: the negative one passed trivially
            # -- an error message contains no "has not acted on" either -- and
            # the positive ones failed, which is why this test read as a known
            # environmental failure rather than as a test asserting nothing.
            ...
            self.assertNotIn(
                ...
                result.stderr,
                "the subprocess could not import helm, so this test asserts nothing",
            )
    ...
    def test_an_automated_notice_is_not_a_request_the_foreman_owes(self) -> None:
        """An `answer` is how route records a request AND how Helm records a notice.

        `route` writes the commander's request to a foreman as an `answer`.
        Several internal paths write one too -- cleanup records one per worker
        to say an escalation is settled. Those are statements, not questions:
        nobody is waiting on the foreman to act on them. Counting them told the
        commander a foreman was ignoring instructions it had never been given,
        and one cleanup sweep of a mature root produced 3,980 of them. The
        writers already mark themselves in the payload; this only had to read it.
        """
        helm_root = self._helm_root("notice-root")
        coordinator, project = self._project_root(helm_root, "notice-project")
        command = shlex.join([sys.executable, "-c", "import time; time.sleep(600)"])
        self._route(helm_root, project["id"], "a real request", "--command", command)
        foreman = coordinator.foreman_for(project["id"])
        self.assertIsNotNone(foreman)
        coordinator.record_worker_message(foreman["id"], "status", "picked that up")
        self.assertEqual(coordinator.pending_foreman_requests(project["id"]), [])
        ...
        with coordinator.store.locked() as data:
            coordinator._message(
                data,
                data["projects"][project["id"]],
                data["tasks"][foreman["task_id"]],
                data["workers"][foreman["id"]],
                "answer",
                "Resolved by cleanup: the commander cleared this task's residue.",
                {"source": "cleanup"},
            )
        self.assertEqual(
            coordinator.pending_foreman_requests(project["id"]),
            [],
            "an automated notice must not read as a request awaiting an answer",
        )

        self._route(helm_root, project["id"], "a second real request")
        pending = coordinator.pending_foreman_requests(project["id"])
        self.assertTrue(
            any("second real request" in item["text"] for item in pending),
            f"a real routed request must still surface: {pending}",
        )
