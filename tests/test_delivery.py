...
class DeliveryTests(HelmTestCase):
    ...
    def test_pr_delivery_stays_open_until_the_pr_is_merged(self) -> None:
        ...
        # Still refused -- but the refusal has to say what to do about it.
        # A pr-open record is the one status here that is routinely stale
        # rather than genuinely unfinished, because nothing polls the PR after
        # Helm hands it over. Saying only "preserve work awaiting approval"
        # about work that has already merged is a dead end, so the message
        # names the command that settles the record.
        with self.assertRaisesRegex(SafetyError, r"pr-sync"):
            self.coordinator.cleanup_task(task["id"])
