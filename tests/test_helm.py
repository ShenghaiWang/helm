...
class HelmCoordinatorTests(unittest.TestCase):
    ...
    def test_a_worker_and_a_project_file_can_never_create_a_grant(self) -> None:
        ...
        # message: it moves the task to approval-needed and stops there.
    ...
    def test_a_review_diffs_from_where_the_task_started_not_the_merge_base(self) -> None:
        """A task branch is cut from the project's HEAD, not from the base branch.

        HEAD can already carry work nobody has merged, and the merge-base sits
        behind it -- so the diff picks up commits the author never wrote. That
        is not theoretical: the first review that actually ran spent its whole
        verdict on a stranger's offline-recording commit, two commits and
        fourteen files where the author wrote one and four. Helm records the
        commit the worktree was cut from; the review measures from there.
        """
