...
class HerdrTests(HelmTestCase):
    ...
    def test_subprocess_herdr_client_sends_the_documented_cli_argv(self) -> None:
        ...
        # The pane's shell must not stop to ask anything before the launch
        # command is typed into it: an update prompt ate the first keystroke.
        self.assertIn("DISABLE_AUTO_UPDATE=true", calls[1])
        self.assertEqual(calls[1][calls[1].index("DISABLE_AUTO_UPDATE=true") - 1], "--env")
...
class AGarbledBriefIsNotAVerdictTests(HelmTestCase):
    """The pane drew the brief with characters dropped, and Helm read it back."""
    ...
    BRIEF = (
        ...
        "continues -- followed by your findings.\nTHE COMPLETE DIFF HAS ALREADY BEEN "
        "COMPUTED FOR YOU: read it."
    )

    def test_a_dropped_character_rendering_of_the_instruction_is_skipped(self) -> None:
        garbled = (
            "CHANGES-REQUESTED --Helm reads that word to decide wheterthe loop "
            "oninues --followedby your findings."
        )
        self.assertTrue(HerdrAdapter._is_garbled_fragment_of(garbled, " ".join(self.BRIEF.split())))
        real = "CHANGES-REQUESTED -- permissions.ts still exports ensureMicrophonePermission; the idle-stream suite has no case for the removed path."
        self.assertFalse(HerdrAdapter._is_garbled_fragment_of(real, " ".join(self.BRIEF.split())))

    def test_recovery_reads_the_reviewers_own_line_and_not_the_drawn_brief(self) -> None:
        root = self.repo("garbled")
        project = self.coordinator.register_project("Garbled", str(root), project_id="garbled")
        task = self.coordinator.create_task(project["id"], self.BRIEF, role="reviewer", read_only=True)
        herdr = FakeHerdr()
        adapter = HerdrAdapter(self.coordinator, herdr)
        worker = adapter.launch_task(task["id"], [sys.executable, "-c", ""], wait=False)
        ...
        log.write_text(
            "[helm] worker started\n"
            "CHANGES-REQUESTED --Helm reads that word to decide wheterthe loop oninues --followedby your findings.\n"
            "TE COMPLTE DIFFHAS ALREADY BEEN COMPUTED FOR YOU: read it.\n",
            encoding="utf-8",
        )
        self.assertIsNone(adapter._verdict_from_output(worker["id"], 0, self.BRIEF))
        with log.open("a", encoding="utf-8") as handle:
            handle.write("APPROVED -- the deletion is complete and nothing else referenced the command.\n")
