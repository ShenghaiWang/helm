...
class CliStatusTests(HelmTestCase):
    ...
    def test_a_cleaned_up_failure_stops_asking_for_a_decision(self) -> None:
        """The item offers three ways out, and cleanup removes all three.

        A failed task keeps `status == "failed"` forever -- cleanup releases
        the worktree and the branch but never rewrites the status -- so the
        decision it raised stayed open with nothing behind it. "Retry it,
        continue it with a new worker, or clean it up" is unanswerable once
        the worktree is gone: `continue` reuses that worktree, and the cleanup
        it suggests has already happened. On this root 253 such items had
        accumulated, more than half the standing backlog, and every cleanup
        added another. An attention list nobody can empty is one nobody reads.
        """
        root = self.repo("cleanedfailure")
        project = self.coordinator.register_project(
            "CleanedFailure", str(root), project_id="cleanedfailure"
        )
        ...
        self.assertTrue(
            any(task["id"] in item["text"] for item in self.coordinator.open_action_items()),
            "a failure should ask for a decision while it still holds a worktree",
        )

        self.coordinator.cleanup_task(task["id"])

        remaining = self.coordinator.open_action_items()
        self.assertFalse(
            any(task["id"] in item["text"] for item in remaining),
            f"a cleaned-up failure still asks for a decision: {remaining}",
        )
        # Still failed -- this closes the question, it does not rewrite history.
        self.assertEqual(
...
class CommanderAskTests(HelmTestCase):
    """Helm could measure what its workers cost a human, not what it cost one.

    Every worker blocker, question and approval request is already recorded.
    A coordinator putting eight questions to the commander in an evening left
    no trace at all -- and under an evaluation that counts human attention,
    the coordinator is the thing being evaluated.
    """

    def test_an_ask_belonging_to_no_project_is_still_recorded(self) -> None:
        """The asks that cost most often belong to no project.

        "Which of these should I pick up next" is pure coordinator overhead
        and names no project. Requiring a project id would have excluded
        exactly the half this record exists to measure -- and Helm itself is
        not a project under `projects/`, so its own work could not be recorded
        either.
        """
        self.coordinator.record_commander_ask(
            "ambiguity", "which of these should I take next"
        )
        counts = self.coordinator.commander_asks()
        self.assertEqual(counts["recorded"]["ambiguity"], 1)
        self.assertEqual(counts["recorded_total"], 1)

    def test_an_unknown_reason_is_refused(self) -> None:
        """The reasons are not interchangeable, so they are not free text.

        An `authorization` ask is Helm working as designed; an `ambiguity` ask
        is a cost the coordinator could have absorbed. A free-text reason would
        let the two blur, and a total that mixes them makes good behaviour and
        bad behaviour look identical.
        """
        with self.assertRaises(HelmError):
            self.coordinator.record_commander_ask("because", "why not")

    def test_recorded_and_derived_are_reported_apart(self) -> None:
        """A single total would hide the one thing worth knowing.

        `derived` needs nobody's honesty -- a decided gate proves a human was
        asked. `recorded` is only as complete as the coordinator marking it.
        If recorded sits far below derived, every rate computed from it is
        wrong in the direction that flatters Helm, which is the direction
        nobody checks.
        """
        root = self.repo("askscope")
        project = self.coordinator.register_project(
            "AskScope", str(root), project_id="askscope"
        )
        self.coordinator.record_commander_ask(
            "authorization", "push this branch?", project_id=project["id"]
        )
        counts = self.coordinator.commander_asks()
        self.assertIn("recorded", counts)
        self.assertIn("derived", counts)
        self.assertNotIn("total", counts, "the two provenances must not be summed")
        self.assertEqual(counts["recorded"]["authorization"], 1)

    def test_a_project_scoped_query_excludes_other_projects(self) -> None:
        first = self.repo("askone")
        second = self.repo("asktwo")
        one = self.coordinator.register_project("One", str(first), project_id="askone")
        two = self.coordinator.register_project("Two", str(second), project_id="asktwo")
        self.coordinator.record_commander_ask(
            "escalation", "worker stuck", project_id=one["id"]
        )
        self.assertEqual(
            self.coordinator.commander_asks(two["id"])["recorded"]["escalation"], 0
        )
        self.assertEqual(
            self.coordinator.commander_asks(one["id"])["recorded"]["escalation"], 1
        )


class WatchdogHeadlineTests(HelmTestCase):
    """The notification has to say what happened, not how many things happened.
    ...
    a notification was sent, so the chain reads as working.
    """

    def test_the_headline_carries_the_item_not_the_count(self) -> None:
    ...
    def test_a_footer_only_list_does_not_become_the_headline(self) -> None:
    ...
    def test_empty_text_still_produces_something_sayable(self) -> None:
        ...
        self.assertTrue(_headline(""))
        self.assertTrue(_headline("\n\n"))

    def test_notify_does_not_claim_a_success_it_never_checked(self) -> None:
        """It returned True unconditionally, whatever the notifier did.

        True now means "handed off without error" -- still not "the human saw
        it", because macOS drops a banner in silence when the scripting host
        lacks permission and exits 0 anyway. That limit is documented at the
        call rather than papered over.
        """
        from unittest import mock
        import helm.watchdog as watchdog

        with mock.patch.object(watchdog.shutil, "which", return_value="/usr/bin/osascript"), \
             mock.patch.object(
                 watchdog.subprocess, "run",
                 return_value=mock.Mock(returncode=1),
             ):
            self.assertFalse(
                watchdog._notify("Helm", "anything"),
                "a notifier that exited non-zero must not report success",
            )
