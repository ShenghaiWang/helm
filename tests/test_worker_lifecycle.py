...
from helm import core, launching
from helm.core import Coordinator, HelmError, StateStore

from tests.support import HelmTestCase, wait_for_exit
...
class WorkerLifecycleConvergenceTests(HelmTestCase):
    ...
    def test_polling_reaps_a_local_child_rather_than_leaving_a_zombie(self) -> None:
        ...
        with self.assertRaises(ChildProcessError):
            # Never this process's child to reap: the runner detaches at
            # launch and init collects it, which is what keeps a caller's
            # process-tree walk from reaching the worker.
    ...
    def test_a_fresh_workspace_is_trusted_before_the_runtime_asks(self) -> None:
        ...
        # `_pretrust_workspace` reads this from its own module, so setting the
        # re-export on `core` would rebind a name nothing looks at.
        original = launching._TRUST_CONFIGS
        launching._TRUST_CONFIGS = {"claude": (config, "hasTrustDialogAccepted")}
        try:
            ...
            launching._TRUST_CONFIGS = {"claude": (Path("/nonexistent/dir/x.json"), "k")}
            core._pretrust_workspace("claude", workspace)
        finally:
            launching._TRUST_CONFIGS = original
...
class VanishedSurfaceOverLiveProcessTests(HelmTestCase):
    """A closed pane is not a finished agent.

    A provider-launched worker had no pid on its record, so liveness came only
    from asking the provider about its *pane*. Closing one recorded the worker
    `failed` and its task `failed` while the agent kept running: `worker
    answer` then refused it as gone, `worker stop` had nothing to signal, and
    the process could only be killed by hand.
    """

    def _external(self, name: str) -> tuple[dict, dict]:
        root = self.repo(name)
        project = self.coordinator.register_project(
            name.title(), str(root), project_id=name
        )
    ...
    def test_the_scan_finds_a_live_agent_by_its_own_state_path(self) -> None:
        """And the bare worker id is deliberately not enough to match.

        `helm worker answer <id>` carries the bare id too, so matching that
        would let a coordinator command be mistaken for the worker it is
        addressing.
        """
        _, worker = self._external("scanfind")
        state = self.coordinator.store.directory
        marker = str(Path(state) / "workers" / worker["id"] / "context.json")

        agent = subprocess.Popen(
            [sys.executable, "-c", "import time; time.sleep(120)", marker]
        )
        self.addCleanup(self._reap, agent)
        for _ in range(50):
            if core._scan_worker_pid(worker["id"], state) is not None:
                break
            time.sleep(0.1)

        self.assertEqual(core._scan_worker_pid(worker["id"], state), agent.pid)

        bare_only = subprocess.Popen(
            [sys.executable, "-c", "import time; time.sleep(120)", worker["id"]]
        )
        self.addCleanup(self._reap, bare_only)
        self.assertNotEqual(core._scan_worker_pid(worker["id"], state), bare_only.pid)

        agent.kill()
        agent.wait()
        for _ in range(50):
            if core._scan_worker_pid(worker["id"], state) is None:
                break
            time.sleep(0.1)
        self.assertIsNone(core._scan_worker_pid(worker["id"], state))

    def test_a_dead_process_behind_a_gone_pane_still_fails_the_task(self) -> None:
        """The change must not spare a worker that is genuinely gone."""
        task, worker = self._external("gonepane")

        self.coordinator.mark_worker_lost(worker["id"], "pane disappeared")

        data = self.coordinator.store.load()
        self.assertEqual(data["workers"][worker["id"]]["status"], "failed")
        self.assertEqual(data["tasks"][task["id"]]["status"], "failed")

    def test_an_orphaned_worker_keeps_its_task_alive_and_says_where_it_went(self) -> None:
        task, worker = self._external("orphaned")

        self.coordinator.mark_worker_orphaned(worker["id"])

        data = self.coordinator.store.load()
        record = data["workers"][worker["id"]]
        self.assertEqual(record["status"], "running")
        self.assertTrue(record.get("orphaned_at"))
        # Not a terminal state: the agent may still finish and report, and
        # failing here would also abandon a hold the session can still spend.
        self.assertEqual(data["tasks"][task["id"]]["status"], "running")
        said = [
            m for m in data["messages"]
            if m.get("worker_id") == worker["id"] and "still running" in (m.get("text") or "")
        ]
        self.assertTrue(said, "an orphaned worker must say so; it has no visible surface")

    def test_stopping_an_orphan_signals_the_process_the_record_never_held(self) -> None:
        """The pid is found at stop time, so a stop that claims to stop, does."""
        _, worker = self._external("stoporphan")
        state = self.coordinator.store.directory
        marker = str(Path(state) / "workers" / worker["id"] / "context.json")
        ...
        self.assertIsNone(self.coordinator.store.load()["workers"][worker["id"]]["pid"])
        stopped = self.coordinator.stop_worker(worker["id"], "orphan test")

        self.assertTrue(stopped.get("signalled"))
        for _ in range(50):
            if agent.poll() is not None:
                break
            time.sleep(0.1)
        self.assertIsNotNone(agent.poll(), "the agent behind the gone pane must be signalled")
...
class UnreapedChildrenTests(HelmTestCase):
    """A worker that dies without reporting must stop reading as alive.

    Helm disowns a child it must not block on, which means nothing ever waits
    on it. Once such a child exits it becomes a zombie, and a zombie is still
    in the process table -- `os.kill(pid, 0)` succeeds for one. So the liveness
    check that decides "the session is gone and left no return code" was being
    asked about a corpse and answered "still running", for as long as the
    coordinator lived.
    """
    ...
    def test_a_worker_that_died_without_reporting_is_settled(self) -> None:
        root = self.repo("unreaped")
        project = self.coordinator.register_project(
            "Unreaped", str(root), project_id="unreaped"
        )
        task = self.coordinator.create_task(project["id"], "dies quietly")
        worker = self.coordinator.launch_worker(
            task["id"], [sys.executable, "-c", ""], wait=False
        )
        self.coordinator.record_worker_message(worker["id"], "result", "done")

        # Wait for the child to become a zombie WITHOUT reaping it here --
        # reaping in the test would do the fix's job and the test would pass
        # either way.
        deadline = time.monotonic() + 10
        while time.monotonic() < deadline and not self._is_zombie(worker["pid"]):
            time.sleep(0.05)
        if not self._is_zombie(worker["pid"]):
            self.skipTest("child was reaped by something else; nothing to assert")

        # The zombie is the whole point: already dead, still answering to
        # os.kill(pid, 0), which is what `_pid_alive` asks.
        self.assertTrue(
            self.coordinator._pid_alive(worker["pid"]),
            "a zombie should still answer os.kill(pid, 0) -- if not, this test "
            "is no longer reproducing the condition it was written for",
        )
        # No exit record: the runner died before writing one, which is what a
        # kill, an OOM, or a crash mid-write leaves behind.
        ...
        settled = self.coordinator.poll_worker(worker["id"])
        self.assertTrue(
            settled.get("exit_observed"),
            "a worker whose process is gone was never observed to have exited, "
            "so its task stays running for the life of the coordinator",
        )


class ProviderWorkersDieTooTests(HelmTestCase):
    """A pane is not a process: a Herdr worker whose agent is gone is gone."""

    def test_a_herdr_worker_with_a_dead_pid_and_no_record_fails_its_task(self) -> None:
        from helm.herdr import HerdrAdapter
        from tests.support import FakeHerdr
        root = self.repo("slept")
        project = self.coordinator.register_project("Slept", str(root), project_id="slept")
        task = self.coordinator.create_task(project["id"], "survive the night")
        herdr = FakeHerdr()
        adapter = HerdrAdapter(self.coordinator, herdr)
        worker = adapter.launch_task(task["id"], [sys.executable, "-c", ""], wait=False)
        # The agent's own process, adopted at launch; then the OS kills it.
        agent = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(60)"])
        with self.state.locked() as data:
            data["workers"][worker["id"]]["pid"] = agent.pid
        self.assertTrue(adapter._provider_worker_alive({**worker, "pid": agent.pid}))
        agent.kill()
        agent.wait()
        self.assertFalse(adapter._provider_worker_alive({**worker, "pid": agent.pid}))
        settled = self.coordinator.poll_worker(worker["id"])
        self.assertEqual(settled["status"], "failed")
        self.assertTrue(settled["exit_observed"])
        self.assertEqual(self.coordinator.inspect_task(task["id"])["task"]["status"], "failed")
        kinds = [m["kind"] for m in self.coordinator.inspect_task(task["id"])["messages"]]
        self.assertIn("failure", kinds)

    def test_stopping_a_settled_worker_ends_its_live_session_and_records_the_exit(self) -> None:
        """A result was pushed, the pane stayed open, then somebody ran stop.

        Stop used to leave a live process alone and record nothing for a
        worker that was not `running`; the adapter closed the pane a moment
        later, the process died unrecorded, and `helm task cleanup` then
        refused the task by pointing at the very stop that had just run.
        """
        from helm.herdr import HerdrAdapter
        from tests.support import FakeHerdr
        root = self.repo("lingering")
        project = self.coordinator.register_project("Lingering", str(root), project_id="lingering")
        task = self.coordinator.create_task(project["id"], "finish, then linger")
        herdr = FakeHerdr()
        adapter = HerdrAdapter(self.coordinator, herdr)
        worker = adapter.launch_task(task["id"], [sys.executable, "-c", ""], wait=False)
        agent = subprocess.Popen([sys.executable, "-c", "import time; time.sleep(60)"])
        self.addCleanup(lambda: (agent.kill(), agent.wait()))
        with self.state.locked() as data:
            data["workers"][worker["id"]]["pid"] = agent.pid
        self.coordinator.record_worker_message(
        ...
        stopped = adapter.stop_worker(worker["id"], "tidy up")

        self.assertEqual(stopped["status"], "completed")
        self.assertTrue(stopped["signalled"], "the session was ended, not merely closed over")
