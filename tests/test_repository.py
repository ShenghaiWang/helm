...
class StateSnapshotCacheTests(HelmTestCase):
    """The parse memo in `StateStore.load`, and the ways it could serve a lie.

    Reusing a parse is only safe while every path that could invalidate it
    actually does. These pin the three that matter: a caller mutating what it
    was handed, a writer replacing the file, and the lock path, which must
    never trade freshness for speed.
    """

    def test_a_cache_hit_does_not_hand_back_the_cached_document(self) -> None:
        # Callers mutate what `load` returns -- several do it in place. If a
        # hit handed out the stored dict, the first such caller would edit the
        # cache itself and every later reader in the process would see an
        # invention that was never on disk.
        self.state.save(self.state.empty())
        first = self.state.load()
        first["tasks"]["invented"] = {"id": "invented"}
        second = self.state.load()
        self.assertNotIn("invented", second["tasks"])

    def test_a_write_through_another_store_is_seen_by_the_next_load(self) -> None:
        # Two processes share one root. A memo keyed on something the writer
        # does not change would let a warmed reader miss the write entirely.
        self.state.save(self.state.empty())
        self.state.load()
        other = StateStore(Path(self.temp.name) / "state")
        document = other.load()
        document["config"]["written_by_another_store"] = True
        other.save(document)
        self.assertTrue(self.state.load()["config"].get("written_by_another_store"))

    def test_the_lock_reads_the_document_rather_than_the_cache(self) -> None:
        # `locked` exists so its block is the only writer. Handing it a parse
        # taken before the lock was acquired would give that writer a document
        # from before whoever it just waited for -- the exact update it locked
        # to serialise against.
        self.state.save(self.state.empty())
        self.state.load()
        other = StateStore(Path(self.temp.name) / "state")
        document = other.load()
        document["config"]["landed_before_the_lock"] = True
        other.save(document)
        with self.state.locked() as data:
            self.assertTrue(data["config"].get("landed_before_the_lock"))

    def test_the_cache_actually_avoids_a_reparse(self) -> None:
        # Without this the three above still pass with the memo deleted, and
        # nothing would notice the speedup silently going away.
        self.state.save(self.state.empty())
        self.state.load()
        with mock.patch.object(
            StateStore, "_read_document", side_effect=AssertionError("re-parsed")
        ):
            self.state.load()
