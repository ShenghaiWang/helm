...
def wait_for_exit(pid: int | None, timeout: float = 10.0) -> None:
    """Wait until a runner pid is gone from the process table.

    The runner detaches from the launcher, so it is not this process's child
    and `os.waitpid` raises; init reaps it. Poll the table instead.
    """
    if not pid:
        return
...
class FakeHerdr:
    def __init__(self, available: bool = True) -> None:
        ...
        # The runtime's own session id for a pane, when the hook reported one.
    ...
    def pane_status(self, pane_id: str) -> dict[str, object]:
        ...
        if pane_id in self.agent_session:
            pane["agent_session"] = {
                "agent": "claude", "kind": "id", "source": "herdr:claude",
                "value": self.agent_session[pane_id],
            }
