...
class ApprovalTests(HelmTestCase):
    ...
    def test_a_paused_task_keeps_its_space_and_releases_it_once_it_finishes(self) -> None:
        ...
        self.assertIn("Approved: publish", herdr.sent_text[-1][1])
...
class AForemansApprovalBindsToItsSubjectTests(HelmTestCase):
    ...
    def test_named_subject_survives_the_project_roots_head_moving(self) -> None:
        root, project, task, foreman_task = self._foreman_asking_to_push("subjected", subject=True)
        ...
        hold = self.coordinator.latest_hold(self.coordinator.inspect_task(foreman_task["id"])["task"])
        self.assertTrue(hold["status"].startswith("authorized"), hold["status"])

    def test_named_subject_still_refuses_when_the_subject_itself_changed(self) -> None:
        root, project, task, foreman_task = self._foreman_asking_to_push("subject-moved", subject=True)
        self.commit_on_task_branch(task, "one more commit nobody was shown")
        with self.assertRaisesRegex(SafetyError, r"changed after it asked"):
            self.coordinator.release_task_hold(foreman_task["id"], action="push", confirm=True)

    def test_a_subject_outside_the_project_is_refused(self) -> None:
        root, project, task, foreman_task = self._foreman_asking_to_push("subject-check", subject=True)
        other_root = self.repo("other")
        other = self.coordinator.register_project("Other", str(other_root), project_id="other")
        other_task = self.coordinator.create_task(other["id"], "elsewhere")
        foreman = next(
            w for w in self.state.load()["workers"].values() if w["task_id"] == foreman_task["id"]
        )
        with self.assertRaisesRegex((SafetyError, HelmError), r"not a task of project"):
            self.coordinator.record_worker_message(
                foreman["id"], "approval-needed", "push something else",
